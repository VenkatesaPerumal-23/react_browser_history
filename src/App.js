...
class App extends Component {
  ...
  onDeleteHistory = id => {
    ...
    const deleteFilteredList = HistoryList.filter(
      eachList => eachList.id !== id,
    )
    this.setState({HistoryList: deleteFilteredList})
  }

  render() {
    const {searchInput, HistoryList} = this.state
    const searchFilteredList = HistoryList.filter(each =>
          ...
          <div className="browser-container">
            <ul>
              {searchFilteredList.length !== 0 ? (
                searchFilteredList.map(eachHistory => (
                  <BrowserHistory
